...
This repo is building **AIKit**, a Swift-first, type-safe, iOS/macOS client framework inspired by Vercel’s **AI SDK** (`./ai-sdk` vendored in this repo for reference). Also vendored in this repo is the **OpenRouter** provider (`./openrouter-provider`).
...
- **Translation-only (no improvisation)**:
  ...
  - The agent’s job is **translation**: always base behavior on the vendored AI SDK’s **code + tests** (`./ai-sdk`).
...
- `ai-sdk/`: vendored JS AI SDK used to reference docs/tests/behavior.
...
- Tests live under `Tests/AIKitCoreTests/...` and should mirror `ai-sdk/packages/ai/src/...` scenario groupings.
