...
extension AIChat {
    ...
    /// Attach content generated by a tool call directly to the corresponding assistant message.
    public func attach(attachments: [ChatAttachment], toToolCallID toolCallID: String) {
        guard !attachments.isEmpty else { return }

        var stored = pendingToolAttachments[toolCallID] ?? []
        stored.append(contentsOf: attachments)
        pendingToolAttachments[toolCallID] = stored

        guard let toolMessageIndex = messages.firstIndex(where: { candidate in
            guard candidate.role == .tool else { return false }
            return candidate.orderedContent.contains { content in
        ...
        let toolMessage = messages[toolMessageIndex]
        messageAttachments[toolMessage.id] = stored

        pendingToolAttachments.removeValue(forKey: toolCallID)
    }
}
