...
public struct ChatToolPart: Sendable, Equatable {
  ...
  public enum State: Sendable, Equatable {
    ...
    /// Mirrors the UI message `output-denied` state, which requires an approval id (approved = false).
...
/// Mirrors the `convertDataPart` return value shape: data parts can optionally become model input parts.
public enum ChatDataPartConversion: Sendable, Equatable {
  case text(MessageTextPart)
  case file(FileContent)
}

/// Converts a `ChatDataPart` into a model message part (or returns `nil` to ignore it).
public typealias ChatDataPartConverter = @Sendable (_ part: ChatDataPart) -> ChatDataPartConversion?
