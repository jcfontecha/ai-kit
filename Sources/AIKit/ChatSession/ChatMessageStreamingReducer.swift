...
enum ChatMessageStreamingReducer {
  ...
  static func apply(
    ...
    case .finishStep:
      ...
      // This matches `processUIMessageStream` semantics and prevents auto-submit predicates
    ...
    case .data(let chunk):
      // Mirrors default `isDataUIMessageChunk` clause:
    ...
    case .file(let file):
      ...
      if let url = URL(string: file.url) {
        // Match behavior: file UI part does not include providerMetadata.
  ...
  private static func mergeObjects(base: JSONValue?, overrides: JSONValue) -> JSONValue? {
    // Mirror `mergeObjects`: deep-merge objects, replace arrays/primitives.
  ...
  private static func upsertToolInputError(from error: ToolInputError, messages: inout [ChatMessage]) {
    ...
    // Match `processUIMessageStream` behavior:
