...
struct ChatSessionInit: Sendable {
  ...
  /// Mirrors `messageMetadataSchema` (`validateTypes`): validates message metadata chunks before merging.
  var validateMessageMetadata: ValidateJSONValue?

  /// Mirrors `dataPartSchemas` (`validateTypes`): keyed by full `data-*` type string.
  var validateDataParts: [String: ValidateJSONValue]?

  /// Optional hook to convert `data-*` UI parts into model message parts (text/file).
  ///
  /// Mirrors `convertDataPart`.
  ...
  ///
  /// This mirrors the setup where the server decides whether responses come from
    ...
    self.init(
      ...
      requestStream: { _, chatMessages, _, _, options, cancellationToken in
        let modelMessages = try await convertToModelMessages(
          chatMessages,
          options: .init(tools: nil, ignoreIncompleteToolCalls: false, convertDataPart: dataPartConverter)
        )
  ...
  /// Convenience initializer for the “remote transport” mode where a server owns inference
  /// and iOS consumes the UI message stream protocol (SSE v1).
  ///
  /// This mirrors `useChat({ transport })` at the API level.
  init(
    id: String? = nil,
...
actor ChatSession {
  ...
  private var convertDataPart: ChatDataPartConverter?
  ...
  private func classifyDisconnect(_ error: Error) -> Bool {
    ...
    // Network-like errors are treated as disconnect.
  ...
  init(_ init: ChatSessionInit) {
    ...
    self.convertDataPart = `init`.convertDataPart
  ...
  private func runRequest(
    ...
    // This matches the behavior: state can still be edited, but the in-flight request uses the snapshot.
