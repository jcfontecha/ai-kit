...
/// A small Swift analogue to the `HttpChatTransport` for non-JS clients.
struct AIUIChatEndpointTransport: Sendable {
  ...
  /// Creates a stream of `TextStreamPart` by POSTing UI messages to a server endpoint that returns
  /// a UI message stream (SSE v1).
  public func requestStream(
    ...
    if let preparedBody = prepared?.body {
      ...
      // Match HttpChatTransport defaults: id/messages are always present and take precedence.
  ...
  ///
  /// Mirrors `HttpChatTransport.reconnectToStream` defaults:
  /// - GET `\(api)/:id/stream`
