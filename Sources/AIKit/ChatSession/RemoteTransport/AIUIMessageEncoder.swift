...
public enum AIUIMessageEncodingError: Error, LocalizedError, Sendable, Equatable {
  ...
  public var errorDescription: String? {
    ...
    case .unsupportedRole(let role):
      return "Unsupported role for UIMessage encoding: \(role)"
    ...
    case .toolOutputDeniedMissingApprovalID(let toolCallID):
      return "Tool part \(toolCallID) is in outputDenied state but is missing the approval id required by UIMessage `output-denied`."
    }
  }
}

/// Encodes AIKit `ChatMessage[]` into UI `UIMessage[]` JSON for Node endpoints.
