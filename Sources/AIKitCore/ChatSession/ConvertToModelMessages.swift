    ...
    case .user:
      ...
      for part in message.parts {
        ...
        case .data(let dataPart):
          // Mirror AI SDK `convertToModelMessages`: data parts are ignored unless a converter is provided.
...
private func convertAssistantBlockToAssistantContent(
  ...
  convertDataPart: (@Sendable (_ part: ChatDataPart) -> ConvertToModelMessagesOptions.ConvertedDataPart?)?
) -> [ModelMessagePart] {
  var content: [ModelMessagePart] = []
  ...
  for part in block {
    ...
    case .tool(let toolPart):
      // Skip tool calls that are still streaming (AI SDK behavior).
      ...
      // Provider-executed tool results (AI SDK parity): include tool results in assistant content and do not add tool-role outputs.
