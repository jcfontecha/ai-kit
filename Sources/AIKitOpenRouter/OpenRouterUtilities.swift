...
func loadOpenRouterAPIKey(apiKey: String?) throws -> String {
  if let apiKey, apiKey.isEmpty == false {
    return apiKey
  }
  if let env = ProcessInfo.processInfo.environment["OPENROUTER_API_KEY"], env.isEmpty == false {
    return env
  }
  throw OpenRouterAPIError(message: "Missing OpenRouter API key.", statusCode: nil)
}
