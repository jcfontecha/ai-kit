...
final class ConvertToModelMessagesTests: XCTestCase {
  ...
  func testUserMessage_dataParts_areSkippedByDefault_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should skip data parts when no converter provided')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testUserMessage_dataParts_convertToTextWhenConverterProvided_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should convert data parts to text when converter provided')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testUserMessage_dataParts_selectivelyConverted_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should selectively convert data parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testUserMessage_dataParts_convertToFileParts_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should convert data parts to file parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testUserMessage_dataParts_multipleTypes_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should handle multiple data parts of different types')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testUserMessage_dataParts_converterDoesNotAffectMessagesWithoutData_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should work with messages that have no data parts')`.
  ...
  func testUserMessage_dataParts_preserveOrder_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in user messages' → 'should preserve order of parts including converted data parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "u1",
        role: .user,
  ...
  func testAssistantMessage_dataParts_convertToTextWhenConverterProvided_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should convert data parts to text when converter provided')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
  ...
  func testAssistantMessage_dataParts_areSkippedByDefault_whenConverterMissing_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should skip data parts when no converter provided')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
  ...
  func testAssistantMessage_dataParts_selectivelyConverted_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should selectively convert data parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
  ...
  func testAssistantMessage_dataParts_convertToFileParts_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should convert data parts to file parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
  ...
  func testAssistantMessage_dataParts_multipleTypes_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should handle multiple data parts of different types')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
  ...
  func testAssistantMessage_dataParts_converterDoesNotAffectMessagesWithoutData_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should work with messages that have no data parts')`.
    let url = URL(string: "https://example.com/image.png")!
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
  ...
  func testAssistantMessage_dataParts_preserveOrder_matchesAISDK() async throws {
    // Mirrors AI SDK `describe('data part conversion' → 'in assistant messages' → 'should preserve order of parts including converted data parts')`.
    let messages: [ChatMessage] = [
      .init(
        id: "a1",
        role: .assistant,
