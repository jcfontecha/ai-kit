...
- `ChatSession` / `ChatMessage` for “useChat-style” interactive sessions (`Sources/AIKit/ChatSession/...`)
- `streamText` / `TextStreamPart` for headless streaming scenarios (`Sources/AIKit/Streaming/StreamText.swift`)
...
- Provide **SwiftUI-first** components (iOS 26+, macOS 26+) aligned with the package platforms in `Package.swift`.
...
- Preserve AI SDK semantics where applicable:
  - tool states (`input-streaming`, `input-available`, `output-*`, approvals) map naturally to `ChatToolPart.State` (`Sources/AIKit/ChatSession/ChatMessage.swift`).
  - approvals flow through `ChatSession.addToolApprovalResponse(...)` (`Sources/AIKit/ChatSession/ChatSession.swift`).
...
- Target: `AIKitElements`
  ...
  - Depends on `AIKit` (for `ChatSession`, `ChatMessage`, `JSONValue`, etc.).
...
- `ChatMessage.role` and `ChatMessage.parts` (`Sources/AIKit/ChatSession/ChatMessage.swift`)
...
- Prefer emitting `ChatDraftMessage` (role + parts) (`Sources/AIKit/ChatSession/ChatDraftMessage.swift`) so apps can do:
...
- `ChatToolPart` + `ChatToolPart.State` + `ChatToolPart.approval` (`Sources/AIKit/ChatSession/ChatMessage.swift`)
...
- Call `await session.addToolApprovalResponse(approvalID:approved:reason:)` (`Sources/AIKit/ChatSession/ChatSession.swift`)
...
- `ChatMessagePart.reasoning(ChatReasoningPart)` has `state: streaming|done` (`Sources/AIKit/ChatSession/ChatMessage.swift`)
